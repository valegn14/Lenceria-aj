    ...
    return (
      <div className="min-h-[50vh] flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-lg sm:text-xl text-gray-600">Descubriendo tus placeres...</p>
        </div>
      </div>
    );
      ...
      {productosFiltrados.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
          {productosFiltrados.map((producto, idx) => (
            <div
              key={idx}
              className="group relative cursor-pointer bg-white overflow-hidden rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2"
              onClick={() =>
                navigate(`/${slugify(producto.id, producto.nombre)}`, { state: producto })
              }
            >
              {/* Image Container - Responsive aspect ratio */}
              <div className="relative aspect-square overflow-hidden rounded-t-2xl bg-gradient-to-br from-pink-50 to-purple-50">
                <div className="relative h-full w-full">
                  ...
                  {producto.rebaja && Number(producto.rebaja) > 0 && (
                    <div className="absolute top-3 left-3 bg-gradient-to-r from-pink-500 to-purple-600 text-white text-xs sm:text-sm font-bold px-3 py-2 rounded-full shadow-lg z-10">
                      -{producto.rebaja}%
                    </div>
                  )}
              ...
              <div className="p-4 sm:p-5 relative z-10 bg-white">
                <h3 className="text-base sm:text-lg font-medium text-gray-800 mb-3 group-hover:text-pink-600 transition-colors line-clamp-2 leading-snug min-h-[2.5rem] sm:min-h-[3rem]">
                  {producto.nombre}
                </h3>
                ...
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button
                    ...
                    className="md:hidden p-3 rounded-full bg-pink-100 hover:bg-pink-200 transition-colors"
                    aria-label="Agregar al carrito"
                  >
                ...
                <p className="text-sm sm:text-base text-gray-600 line-clamp-3 leading-relaxed min-h-[4rem] sm:min-h-[4.5rem]">
                ...
                <button
                  ...
                  className="
                    md:hidden
                    w-full mt-4 py-3
                    bg-gradient-to-r from-pink-500 to-purple-600 text-white text-center font-medium
                    rounded-xl
              ...
              {/* Hover Border Effect */}
              <div className="absolute inset-0 border-2 border-transparent group-hover:border-pink-300 rounded-2xl pointer-events-none transition-all duration-500"></div>
            </div>
          ))}
        </div>
      )}
